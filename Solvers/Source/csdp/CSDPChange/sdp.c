  ...
  while ((relgap > parameters.objtol) || (relpinfeas > parameters.axtol) || (reldinfeas > parameters.atytol))
	 {
	    ...
	    * if he wants to.
	    */
		 ret = user_exit(n,k,C,a,*dobj,*pobj,constant_offset,constraints,X,y,Z,parameters);
	   if (ret != 0)
	     {
	       return(ret);
	     }
...
#pragma omp parallel for schedule(dynamic,64) private(i,j) shared(O,k,ldam)
	   ...
	   */
		...
		*/
		   ret = user_exit(n,k,C,a,affdobj,affpobj,constant_offset,constraints,work1,workvec1,work2,parameters);
	       if (ret != 0) 
		 {
		   *dobj=affdobj;
		   *pobj=affpobj;
