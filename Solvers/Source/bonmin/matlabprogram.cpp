...
bool MatlabProgram::get_variables_types(Index n, VariableType* var_types)
{   
    try 
    {
        // Fill in the structure with the variable types information.
        copymemory(options.variable_types(),var_types,n);
        return true;
    } 
...
bool MatlabProgram::get_variables_linearity(Index n, Ipopt::TNLP::LinearityType* var_types)
{
    try 
    {
        // Fill in the structure with the variable types information.
        copymemory(options.variable_linearity(),var_types,n);
        return true;
    } 
    catch (std::exception& error) 
    {
...
bool MatlabProgram::get_constraints_linearity(Index m, Ipopt::TNLP::LinearityType* const_types)
{
    try 
    {
        // Fill in the structure with the constraints types information.
        copymemory(options.constraint_linearity(),const_types,m);
        return true;
    } 
    catch (std::exception& error) 
    {
...
bool MatlabProgram::get_nlp_info (int& n, int& m, int& sizeOfJ, int& sizeOfH, 
  ...
  try {
	...
	throw MatlabException("You need to specify the callback functions for computing the Jacobian and the sparsity structure of the Jacobian");
    ...
    if (!options.bonminOptions().useQuasiNewton()) {
      if (!funcs.hessianFuncIsAvailable())
	throw MatlabException("You need to specify the callback functions for computing the Hessian and the sparsity structure of the Hessian");
...
bool MatlabProgram::get_starting_point (int n, bool initializeVars, 
  ...
  try {
    ...
    // Check to see whether BONMIN is requesting the initial point for
	...
	throw MatlabException("Initialization of Lagrange multipliers for lower and upper bounds requested but initial values are not supplied");
	...
	throw MatlabException("Initialization of Lagrange multipliers for constraints are requested but initial values are not provided");
...
bool MatlabProgram::eval_g (int n, const double* vars, bool ignore, int m, 
  ...
  try {
	...
	throw MatlabException("You need to specify the callback function for computing the constraints");
      x.inject(vars);
  ...
  try {
	...
	J = funcs.getJacobianStructure(n,m,auxdata);
 	if (J->numelems() != sizeOfJ)
 	  throw MatlabException("The constraint Jacobian passed back from the MATLAB routine has an incorrect number of nonzero entries");
...
bool MatlabProgram::eval_h (int n, const double* vars, bool ignore, 
  ...
  try {
	...
	throw MatlabException("The Hessian passed back from the MATLAB routine has an incorrect number of nonzero entries");
...
void MatlabProgram::finalize_solution(TMINLP::SolverReturn status,
