...
slug: "/reference/options/optiset/"
---
...
To see which solvers contain extra option methods, check the `sOpts` column when using `checkSolver('config')`.

## dynamicOpts
This field allows you to supply settings specific to dynamic optimization via structure. This structure is generated using [`optidynset`](./optidynset.md) and is passed to `optiset` in the same way as solver options, as above.
