...
slug: "/examples/dynamic-optimization/dnls/"
---
...
## Example 4: Calculating Goodness of Fit
As described in the [NLS Examples](../problem-types/nls.md#nls-stats), from OPTI v2.10 it is now possible to calculate the goodness of fit for curve fitting and parameter estimation problems. This functionality applies to most dynamic system parameter estimation problems, and is implemented in an identical way via `fitStats`:

```matlab
% Generate Fitting Data with Noise
tm  = 0:0.1:2;                  %measurement times
odeInt = @(t,z) ode(t,z,p);     %ODE function for ODE45
[~,zm] = ode45(odeInt,tm,z0);   %Solve ODEs
zm = zm + 0.25*randn(size(zm)); %Random Noise
% Initial Conditions
z0 = [-1.5;1.25;1];
...
![plot dnls stats](/img/opti/plot_dnls_stats.png)

<small>**NOTE:**</small> I am not 100% sure that the fit statistics are calculated correctly for systems of differential equations. I have treated them (from a statistical point of a view) as the same as algebraic equations, however due to the integrator, the data is highly correlated, and therefore they may not be valid. If anyone has more experience here advice would be appreciated!
...
## Example 6: Multiple Sampling Rate Measurements
...
## Example 7: 'Winding Back' the Integrator
...
## Example 8: Repeated Measurements
...
## Example 9: Supplying Analytical Derivatives
